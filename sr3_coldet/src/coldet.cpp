      ...
      {
        {
          for(int i=0;i<tnum2;i++)
          {
            ...
            for(int j=0;j<tnum1;j++)
            {
              ...
              if (tt.intersect(*bt1)) 
              {
                m_ColTri1[m_NumTriangles]=*bt1;
                m_iColTri1=getTriangleIndex(bt1);
                m_ColTri2[m_NumTriangles]=tt;
...
bool CollisionModel3DImpl::getCollidingTriangles(int nTri, float t1[9], float t2[9], bool ModelSpace)
{
  if (ModelSpace)
  {
    if (t1!=NULL)
    {
      *((Vector3D*)&t1[0]) = m_ColTri1[nTri].v1;
      *((Vector3D*)&t1[3]) = m_ColTri1[nTri].v2;
      *((Vector3D*)&t1[6]) = m_ColTri1[nTri].v3;
    }
    if (t2!=NULL)
    {
      *((Vector3D*)&t2[0]) = m_ColTri2[nTri].v1;
      *((Vector3D*)&t2[3]) = m_ColTri2[nTri].v2;
      *((Vector3D*)&t2[6]) = m_ColTri2[nTri].v3;
    }
  }
  else
  {
    if (t1!=NULL)
    {
      *((Vector3D*)&t1[0]) = Transform(m_ColTri1[nTri].v1,m_Transform);
      *((Vector3D*)&t1[3]) = Transform(m_ColTri1[nTri].v2,m_Transform);
      *((Vector3D*)&t1[6]) = Transform(m_ColTri1[nTri].v3,m_Transform);
    }
    if (t2!=NULL)
    {
      *((Vector3D*)&t2[0]) = Transform(m_ColTri2[nTri].v1,m_Transform);
      *((Vector3D*)&t2[3]) = Transform(m_ColTri2[nTri].v2,m_Transform);
      *((Vector3D*)&t2[6]) = Transform(m_ColTri2[nTri].v3,m_Transform);
    }
  }
  return true;
}
...
bool CollisionModel3DImpl::getCollisionPoint(int nPnt, float p[3], bool ModelSpace)
{
  ...
  switch (m_ColType) 
  {
    case Models: v=my_tri_tri_intersect(m_ColTri1[nPnt],m_ColTri2[nPnt]); break;
    case Sphere:
