...
public:
  ...
  bool getCollidingTriangles(int nTri, float t1[9], float t2[9], bool ModelSpace);
  bool getCollidingTriangles(int& t1, int& t2);
  bool getCollisionPoint(int nPnt, float p[3], bool ModelSpace);

  int  getNumCollisions(void)		{ return m_NumTriangles; }


  int getTriangleIndex(BoxedTriangle* bt)
  {
   // return int(bt-m_Triangles.begin());
   //return int();                                                  //hacked to work TECAN
  }
  ...
  */
  ...
  int						 m_NumTriangles;
  Triangle                   m_ColTri1[64],m_ColTri2[64];
